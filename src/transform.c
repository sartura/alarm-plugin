...
snabb_datastore_to_sysrepo(ctx_t *ctx) {
	...
	snprintf(message, SNABB_MESSAGE_MAX, "get-alarms-state {path '/'; schema %s;}", ctx->yang_model);
	command = SB_GET;
...
snabb_state_data_to_sysrepo(ctx_t *ctx, char *xpath, sr_val_t **values, size_t *values_cnt) {
	...
	snprintf(message, SNABB_MESSAGE_MAX, "get-alarms-state {path '%s'; schema %s;}", action->snabb_xpath, ctx->yang_model);
	free_action(action);
	action = NULL;
	command = SB_GET;
