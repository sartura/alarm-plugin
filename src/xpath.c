...
transform_data_to_array(ctx_t *ctx, char *xpath, char *data, struct lyd_node **node) {
	...
	while ((token = strsep(&data, "\n")) != NULL) {
		...
		if (0 == strlen(token)) {
			...
			parent = (NULL != parent->parent) ? parent->parent : parent;
			continue;
		} else {
			...
			*tmp = '\0';
			...
			if ('{' == *last) {
				...
				*last = '\0';
				...
				if (NULL == check) {
					ERR("lyd_new_leaf failed for node %s data %s", token, tmp);
					ERR("libyang error msg: %s", ly_errmsg());
					rc = SR_ERR_INTERNAL;
					goto error;
				}
			}
	...
	char *strp = NULL;
	lyd_print_mem(&strp, top_parent, LYD_JSON, 0);
	INF("%s", strp);
